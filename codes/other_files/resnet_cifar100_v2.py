...
valset = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=transform_test)
valid_size = 0.2
  ...
  for epoch in range(1, n_epochs+1):
      ...
      with torch.no_grad():
          ...
          if network_learned:
              ...
              torch.save(model.state_dict(), f'/home/rafayel.veziryan/cnn_exp/results/cifar100_2/{model._get_name()}_bst.pt')
...
class ResNet18_with_cable_eq_v2(nn.Module):
    def __init__(
        self, 
        img_channels: int,
        num_layers: int,
        ...
        super(ResNet18_with_cable_eq_v2, self).__init__()
        # Initialazing kernels
        
        #self.filter_1 = filter1
        #self.filter_2 = filter2
        #self.filter_3 = filter3
        # 2nd
        '''
        self.register_buffer("filter1", filter1)
        self.register_buffer("filter2", filter2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.mylayer_1(x)
        #x = my_layer(x, self.weight2_1, self.weight2_2, self.weight2_3, self.filter_1, self.filter_2, self.filter_3, self.bn02)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
class ResNet18_with_cable_eq_3(nn.Module):
    def __init__(
        ...
        self.bn01 = nn.BatchNorm2d(img_channels)
        self.bn02 = nn.BatchNorm2d(img_channels)
        self.bn03 = nn.BatchNorm2d(img_channels)
        self.bn1 = nn.BatchNorm2d(self.in_channels)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0])
...
class ResNet18_with_cable_eq_4(nn.Module):
    def __init__(
        ...
        self.bn02 = nn.BatchNorm2d(img_channels)
        self.bn03 = nn.BatchNorm2d(img_channels)
        self.bn04 = nn.BatchNorm2d(img_channels)
        self.bn1 = nn.BatchNorm2d(self.in_channels)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0])
...
class ResNet18_with_cable_eq_5(nn.Module):
    def __init__(
        ...
        self.bn03 = nn.BatchNorm2d(img_channels)
        self.bn04 = nn.BatchNorm2d(img_channels)
        self.bn05 = nn.BatchNorm2d(img_channels)
        self.bn1 = nn.BatchNorm2d(self.in_channels)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0])
...
model_original = ResNet18(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
model_cable_eq = ResNet18_with_cable_eq_v2(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
model_cable_eq_2 = ResNet18_with_cable_eq_2(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
model_cable_eq_3 = ResNet18_with_cable_eq_3(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
model_cable_eq_4 = ResNet18_with_cable_eq_4(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
model_cable_eq_5 = ResNet18_with_cable_eq_5(img_channels=3, num_layers=18, block=BasicBlock, num_classes=100).to(device)
...
for model in models:
    ...
    torch.save(model.state_dict(), f'/home/rafayel.veziryan/cnn_exp/results/cifar100_2/{model._get_name()}_best.pt')
    model_cable_eq_dict ={}
    ...
    with open(f'/home/rafayel.veziryan/cnn_exp/results/cifar100_2/{str(model._get_name())}_bn.pkl', 'wb') as fp:
