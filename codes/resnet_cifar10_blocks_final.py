  ...
  for epoch in range(1, n_epochs+1):
      ...
      with torch.no_grad():
          ...
          if network_learned:
              ...
              torch.save(model.state_dict(), f'/home/rafayel.veziryan/cnn_exp/results/cifar10_lr_0.5_blocks_final/{model._get_name()}_bst.pt')
...
class ResNet18_with_cable_eq(nn.Module):
    def __init__(
        ...
        self.my_layer = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.in_channels = 64
        # All ResNets (18 to 152) contain a Conv2d => BN => ReLU for the first
        # three layers. Here, kernel size is 7.
        self.conv1 = nn.Conv2d(
...
class ResNet18_with_cable_eq_2(nn.Module):
    def __init__(
        ...
        self.my_layer1 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer2 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.in_channels = 64
        # All ResNets (18 to 152) contain a Conv2d => BN => ReLU for the first
        # three layers. Here, kernel size is 7.
        self.conv1 = nn.Conv2d(
...
class ResNet18_with_cable_eq_3(nn.Module):
    def __init__(
        ...
        self.my_layer1 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer2 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer3 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.in_channels = 64
        # All ResNets (18 to 152) contain a Conv2d => BN => ReLU for the first
        # three layers. Here, kernel size is 7.
        self.conv1 = nn.Conv2d(
...
class ResNet18_with_cable_eq_4(nn.Module):
    def __init__(
        ...
        self.my_layer2 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer3 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer4 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.in_channels = 64
        # All ResNets (18 to 152) contain a Conv2d => BN => ReLU for the first
        # three layers. Here, kernel size is 7.
        self.conv1 = nn.Conv2d(
...
class ResNet18_with_cable_eq_5(nn.Module):
    def __init__(
        ...
        self.my_layer3 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer4 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.my_layer5 = self._make_layer_pde(block=my_block, out_channels=3, blocks=1)
        self.in_channels = 64
        # All ResNets (18 to 152) contain a Conv2d => BN => ReLU for the first
        # three layers. Here, kernel size is 7.
        self.conv1 = nn.Conv2d(
...
for model in models:
    ...
    torch.save(model.state_dict(), f'/home/rafayel.veziryan/cnn_exp/results/cifar10_lr_0.5_blocks_final/{model._get_name()}_best.pt')
    model_cable_eq_dict ={}
    ...
    with open(f'/home/rafayel.veziryan/cnn_exp/results/cifar10_lr_0.5_blocks_final/{str(model._get_name())}_bn.pkl', 'wb') as fp:
